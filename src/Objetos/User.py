class User():
    ...
    def __str__(self):
        return f"User: {self.id}, {self.name}, {self.email}, {self.password}, {self.type}"
    ...
    def cadastrar(self):
        ...
        cursor.execute(f"INSERT INTO users (name, email, password, type) VALUES ('{self.name}', '{self.email}', '{self.password}', '{self.type}')")
        self.connection.commit()
    
    # Atualiza o objeto no banco de dados
    def atualizar(self):
        cursor = self.connection.get_cursor()
        cursor.execute(f"UPDATE users SET name = '{self.name}', email = '{self.email}', password = '{self.password}', type = '{self.type}' WHERE id = {self.id}")
        self.connection.commit()
