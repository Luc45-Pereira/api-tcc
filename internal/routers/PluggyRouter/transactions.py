...
async def get_accounts_of_transactions(payload: token_model = Depends()):
    ...
    usuario = session.query(Database.Account).filter(Database.Account.id_usuario == decoded_token["user"]).first()
    
    url = f"{URL}/accounts?itemId={usuario.pluggy_connection_id}"
...
async def select_account_of_transactions(account_id: str, payload: token_model = Depends()):
    ...
    try:
        decoded_token = jwt.decode(payload.access_token, SECRET_KEY, algorithms=[ALGORITHM])
        session = Session.get_session()
        account_user = session.query(Database.Account).filter(Database.Account.id_usuario == decoded_token['user']).first()
